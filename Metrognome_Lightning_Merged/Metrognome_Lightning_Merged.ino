// Metrognome_Lightning_Merged.ino
// Single-file Metrognome build with weather pages, LIGHT update, QMC5883 compass and SG90 lightning vane.
//
// This sketch avoids Arduino tab-order gremlins by keeping everything in one .ino file.
//
// Hardware assumptions:
//   ESP32-C3 Super Mini
//   Small I2C OLED on GPIO5 SDA / GPIO6 SCL
//   Big SPI OLED on SCK GPIO4, MOSI GPIO7, RST GPIO8, DC GPIO9, CS GPIO10
//   Rotary encoder CLK GPIO0, DT GPIO1, SW GPIO2
//   QMC5883 compass on same I2C bus: SDA GPIO5, SCL GPIO6, VCC 3V3, GND common
//   SG90 servo signal GPIO3, VCC 5V rail, GND common
//
// secrets.h should define your existing WiFi and Stormglass values.
// Optional for direct lightning feed:
//   const char* LIGHTNING_FEED_URL = "http://example.local/lightning.json";
// If LIGHTNING_FEED_URL is absent or empty, the LIGHT page uses a demo strike so hardware can be tested.
...
constexpr float DECLINATION_DEGREES = 0.0;
constexpr float COMPASS_MOUNT_OFFSET = 0.0;
...
bool fetchLightningDirect() {
  ...
  if (url.length() == 0) {
    // Demo mode: strike roughly NW of Bristol/Warrington-ish depending on configured location.
    // Good enough to prove the display, compass and servo path before choosing a real public feed.
