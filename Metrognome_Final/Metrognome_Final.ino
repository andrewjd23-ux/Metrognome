// Metrognome_Final.ino
// Final production Metrognome build.
// Compass removed. Manual-north lightning vane retained.
// Servo signal now uses RX / GPIO20.
//
// Hardware:
//   ESP32-C3 Super Mini
...
//   SG90 servo: signal RX/GPIO20, red 5V rail, brown/black common GND
//
// secrets.h should define WiFi, Stormglass, and location values used by earlier builds.
// Optional lightning feed:
//   const char* LIGHTNING_FEED_URL = "http://example.local/lightning.json";
// If absent or empty, LIGHT uses a demo strike.
// Optional tide feed:
...
constexpr uint8_t PIN_SERVO_SIGNAL = 20;  // RX pin
...
constexpr int SERVO_SCAN_REPEATS = 2;
constexpr int SERVO_SCAN_STEP = 2;
constexpr int SERVO_SCAN_DELAY_MS = 24;

// Manual north mode. Set to the bearing the gnome's FRONT faces.
...
void servoWriteTracked(int angle) {
  currentServoAngle = constrain(angle, SERVO_LEFT_LIMIT, SERVO_RIGHT_LIMIT);
  lightning.servoAngle = currentServoAngle;
  lightningServo.write(currentServoAngle);
}

void moveServoSlowTo(int target, int stepSize = SERVO_SCAN_STEP, int delayMs = SERVO_SCAN_DELAY_MS) {
  target = constrain(target, SERVO_LEFT_LIMIT, SERVO_RIGHT_LIMIT);
  if (currentServoAngle == target) return;

  int dir = (target > currentServoAngle) ? 1 : -1;
  while (currentServoAngle != target) {
    int next = currentServoAngle + (dir * stepSize);
    if ((dir > 0 && next > target) || (dir < 0 && next < target)) next = target;
    servoWriteTracked(next);
    frame++;
    smallStatus();
    if (page == LIGHTNING) drawBig();
    delay(delayMs);
  }
}

void scanThenSettleServo(int targetAngle) {
  servoScanning = true;
  lightning.status = "SCAN";
  apiStatus = "SCAN";
  smallStatus();
  drawBig();

  moveServoSlowTo(SERVO_CENTER, 3, 18);
  for (int i = 0; i < SERVO_SCAN_REPEATS; i++) {
    moveServoSlowTo(SERVO_LEFT_LIMIT, SERVO_SCAN_STEP, SERVO_SCAN_DELAY_MS);
    delay(120);
    moveServoSlowTo(SERVO_RIGHT_LIMIT, SERVO_SCAN_STEP, SERVO_SCAN_DELAY_MS);
    delay(120);
  }
  moveServoSlowTo(targetAngle, SERVO_SCAN_STEP, SERVO_SCAN_DELAY_MS);
...
void updateWifiStatus() {
  ...
  wifiStatus = wifiOk ? "OK" : "FAIL";
  connectedSsid = wifiOk ? WiFi.SSID() : "";
}
...
bool fetchStormglass() {
  if (!wifiOk && !connectBestWiFi()) return false;
  syncTime();
  ...
  if (!http.begin(client, stormglassUrl())) { apiStatus = "HTTP"; return false; }
  ...
  if (err) { apiStatus = "JSON"; Serial.println(err.c_str()); return false; }
  JsonObject hour0 = doc["hours"][0];
  if (hour0.isNull()) { apiStatus = "EMPTY"; return false; }
...
bool fetchTide() {
  ...
  url.trim();
  if (url.length() == 0) { tide.valid = false; tide.status = "NO TIDE URL"; apiStatus = "TIDE URL"; return false; }
  if (!wifiOk && !connectBestWiFi()) { tide.valid = false; tide.status = "WIFI FAIL"; return false; }
  ...
  bool begun = false;
  if (url.startsWith("https://")) { secureClient.setInsecure(); begun = http.begin(secureClient, url); }
  else begun = http.begin(url);
  if (!begun) { tide.valid = false; tide.status = "HTTP"; apiStatus = "TIDE HTTP"; return false; }

  http.addHeader("Accept", "application/json");
  int code = http.GET();
  Serial.print("Tide HTTP code: "); Serial.println(code);
  if (code != 200) { http.end(); tide.valid = false; tide.status = "HTTP ERR"; apiStatus = "TIDE ERR"; return false; }
  String payload = http.getString();
  http.end();
  if (payload.length() == 0) { tide.valid = false; tide.status = "EMPTY"; apiStatus = "TIDE EMPTY"; return false; }

  DynamicJsonDocument doc(32768);
  DeserializationError err = deserializeJson(doc, payload);
  if (err) { tide.valid = false; tide.status = "JSON"; apiStatus = "TIDE JSON"; return false; }
  ...
  else { tide.valid = false; tide.status = "NO ARRAY"; apiStatus = "TIDE ARRAY"; return false; }
  ...
  if (bestTime == "" || isnan(bestLevel)) { tide.valid = false; tide.status = "NO DATA"; apiStatus = "TIDE DATA"; return false; }
...
bool parseLightningJson(const String& payload) {
  DynamicJsonDocument doc(32768);
  DeserializationError err = deserializeJson(doc, payload);
  if (err) { lightning.status = "JSON"; return false; }
  ...
  else { lightning.status = "NO ARRAY"; return false; }
  ...
  for (JsonVariant item : strikes) {
    ...
    if (extractLatLon(item, lat, lon)) {
      ...
      if (c.size() >= 2) { lon = c[0].as<double>(); lat = c[1].as<double>(); }
      else continue;
    } else continue;

    count++;
    float km = distanceKmBetween(METROGNOME_LAT, METROGNOME_LON, lat, lon);
    if (km < bestKm) { bestKm = km; bestLat = lat; bestLon = lon; found = true; }
  }
  if (!found) { lightning.active = false; lightning.status = "NO STRIKE"; return false; }
...
bool fetchLightningDirect() {
  ...
  if (!wifiOk && !connectBestWiFi()) { lightning.status = "WIFI FAIL"; return false; }
  ...
  if (!begun) { lightning.status = "HTTP"; return false; }
  http.addHeader("Accept", "application/json");
  int code = http.GET();
  Serial.print("Lightning HTTP code: "); Serial.println(code);
  if (code != 200) { http.end(); lightning.status = "HTTP ERR"; return false; }
  String payload = http.getString();
  http.end();
  if (payload.length() == 0) { lightning.status = "EMPTY"; return false; }
...
void setupLightningVane() {
  ...
  servoWriteTracked(SERVO_CENTER);
  lightning.status = "READY";
  lightning.headingDeg = readManualHeading();
  Serial.println("Manual-north scanning lightning vane ready on RX/GPIO20.");
}
...
void updateLightningPage() {
  ...
  if (!gotStrike) {
    ...
    lightning.status = "NO TARGET";
    scanThenSettleServo(SERVO_CENTER);
    apiStatus = "LIGHT ERR";
    return;
  }

  float relative = signedRelativeAngle(lightning.azimuthDeg, heading);
  int targetAngle = relativeToServo(relative);
  lightning.relativeDeg = relative;
  lightning.servoAngle = targetAngle;
  lightning.status = "FOUND";
  ...
  Serial.print("Target servo angle: "); Serial.println(targetAngle);

  scanThenSettleServo(targetAngle);
  lightning.status = gotStrike ? "DEMO/OK" : "ERR";
  apiStatus = "LIGHT OK";
}
...
void drawTide() {
  ...
  if (tide.valid) {
    bigDisplay.setFont(u8g2_font_7x14B_tf);
    bigDisplay.setCursor(0, 42); bigDisplay.print(tide.latestLevel, 2); bigDisplay.print("m");
...
void drawLightning() {
  ...
  bigDisplay.setFont(u8g2_font_6x10_tf);
  bigDisplay.drawStr(0, 24, servoScanning ? "Scanning sky" : "Nearest strike");
  bigDisplay.setFont(u8g2_font_7x14B_tf);
  ...
  } else bigDisplay.print(lightning.status);
  bigDisplay.setFont(u8g2_font_5x8_tf);
  ...
  float displayRelative = servoScanning ? (float)(currentServoAngle - SERVO_CENTER) : lightning.relativeDeg;
  float a = (displayRelative - 90.0) * PI / 180.0;
  int px = cx + (int)(cos(a) * 19.0);
  int py = cy + (int)(sin(a) * 19.0);
  bigDisplay.drawLine(cx, cy, px, py);
  bigDisplay.drawDisc(px, py, 2);
  if (!lightning.active && !servoScanning) bigDisplay.drawCircle(cx, cy, ((frame * 3) % 18) + 2);
  bigDisplay.sendBuffer();
}
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("Metrognome Final build: manual north, tide, weather, scanning lightning vane");
