...
Page page=WEATHER;
uint32_t frame=0,lastDraw=0,lastButton=0,pressUntil=0;
int lastClk=HIGH;
...
void smallStatus(){
  ...
  smallDisplay.setCursor(0,15); smallDisplay.print("PAGE "); smallDisplay.print(pageNames[page]);
  smallDisplay.drawStr(0,24,"ENC  OK");
...
void drawSystem(){
  ...
  bigDisplay.setFont(u8g2_font_6x10_tf); bigDisplay.drawStr(0,25,"Dual OLED: OK"); bigDisplay.drawStr(0,37,"Encoder:   OK"); bigDisplay.drawStr(0,49,"WiFi/API:  idle");
  bigDisplay.setCursor(0,61); bigDisplay.print("Uptime: "); bigDisplay.print(millis()/1000); bigDisplay.print("s");
  if(pressNotice){ bigDisplay.drawBox(92,17,34,12); bigDisplay.setDrawColor(0); bigDisplay.drawStr(96,27,"PUSH"); bigDisplay.setDrawColor(1); }
  bigDisplay.sendBuffer();
}
...
void setup(){
  Serial.begin(115200); delay(1000); Serial.println("Metrognome encoder test");
